...
public class AudioSpectrum : MonoBehaviour {
	...
	private void Update()
    {
		...
		foreach (GameObject c in cubeSpawner.cubes)
        {
            if (c == null)
            {
        ...
        if (preProcessSamples && preProcessedSpectralFluxAnalyzer.spectralFluxSamples.Count > 0 && timeTracker > timeStep)
        {
			if (spawnPrePlayCubes)
			{
				timer = Time.time - countFrom;
    ...
    public void spawnCubes(List<SpectralFluxInfo> pointInfo, int curIndex) {
        ...
        if (pointInfo[curIndex].isPeak)
        {
			...
			// 	pointInfo[curIndex].time,
			// 	pointInfo[curIndex].prunedSpectralFlux,
			// 	pointInfo[curIndex].spectralFlux,
